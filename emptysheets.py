...
for sheet in sheets:
    ...
    if sh.max_row is 1:
        ...
        html_content = requests.get(url + '/' + sheet + '/').text
        ...
        # print( xs )
        she = book.get_sheet_by_name( sheet )
        # first_sheet = book.get_sheet_by_name('Sheet')
        # book.remove_sheet(she)
        ...
        for  x in xs[4:]:
            ...
            # for td in tds:
            try:
                record = (tds[0].text,tds[1].text, tds[2].text, tds[3].text, tds[4].text, sheet, None, None, None, None, None )
                rows.append( record )
                # print( td.text )
            except IndexError:
                continue
